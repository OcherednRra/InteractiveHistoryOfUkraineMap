...
export const useMarkerFilterSearch = (markers = [], currentUserEmail = '') => {
  ...
  const getMarkerVisibility = useCallback((marker) => {
    // Базовая прозрачность в зависимости от статуса модерации
    const baseOpacity = (marker.is_moderated === false) ? 0.5 : 1;
    ...
    if (searchQuery.trim()) {
      ...
      if (!matchesTitle && !matchesTags && !matchesDescription) {
        return 0.1; // Скрыть метку (независимо от статуса модерации)
      }
    }
    ...
    // Возвращаем базовую прозрачность с учетом статуса модерации
    return baseOpacity;
  }, [
    searchQuery,
    dateFrom,
  ...
  // ИСПРАВЛЕНО: Отфильтрованные метки для списка результатов - включаем и немодерированные
  const filteredMarkers = useMemo(() => {
    return markers.filter(marker => {
      const visibility = getMarkerVisibility(marker);
      return visibility > 0.1; // Показываем все видимые метки (и модерированные, и немодерированные)
    });
  }, [markers, getMarkerVisibility]);
  ...
  // ИСПРАВЛЕНО: Статистика фильтрации - правильный подсчет видимых меток
  const filterStats = useMemo(() => {
    const visibleCount = Array.from(markerVisibilityMap.values()).filter(opacity => opacity > 0.1).length;
    const fullyVisibleCount = Array.from(markerVisibilityMap.values()).filter(opacity => opacity === 1).length;
    const partiallyVisibleCount = Array.from(markerVisibilityMap.values()).filter(opacity => opacity === 0.5).length;
